...
- `diff.external` removed from `_SAFE_CONFIG_FLAGS` [PH-154] — setting `-c diff.external=` (empty string) causes git to exec `""` when generating unified diffs (`git diff -p`), failing with "cannot run : No such file or directory"; `GIT_CONFIG_NOSYSTEM=1`+`GIT_CONFIG_GLOBAL=/dev/null` already block system/user configs; local `.git/config` write-access implies repo admin access (accepted risk)
