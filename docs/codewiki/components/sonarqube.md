...
- **Per-repo health**: `SonarQubeMetric` gained `repo_id` (FK, nullable) and its unique
  ...
  the per-repo breakdown (`repo_id, repo_slug, repo_name, project_key, the 7 metrics,
  dashboard_url`). PH-249 reads `repo_health`; old consumers reading only `health` are
  unaffected. (The `Board.sonarqube_metric` 1:1 relationship was renamed to
