...
- `src/hooks/useWebSocket.ts:319-324` — hard-coded heuristic that flags tokens starting with `1c7f53fb` as "jarwis-backend token instead of user token" and logs an error during a 1006 close. This is a per-environment fingerprint that will be misleading on any other deployment; revisit if/when tokens rotate. [bootstrap]
